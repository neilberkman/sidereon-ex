defmodule Sidereon.CoordinatesTest do
  ...
  describe "Sidereon.ground_track/2 (high-level)" do
    test "returns one geodetic sub-point per datetime, matching geodetic/2" do
      {:ok, tle} = Sidereon.parse_tle(@iss_line1, @iss_line2)
      base = ~U[2024-01-01 12:00:00Z]
      times = for s <- 0..600//120, do: DateTime.add(base, s, :second)

      {:ok, track} = Sidereon.ground_track(tle, times)

      assert length(track) == length(times)
      assert Enum.all?(track, &match?(%Sidereon.Geodetic{}, &1))

      # Each sub-point must match the single-epoch Sidereon.geodetic/2 reduction.
      # ground_track routes through the core converter which round-trips through
      # radians/meters, so allow a sub-micro tolerance rather than exact bits.
      for {dt, point} <- Enum.zip(times, track) do
        {:ok, geo} = Sidereon.geodetic(tle, dt)
        assert_in_delta point.latitude, geo.latitude, 1.0e-6
        assert_in_delta point.longitude, geo.longitude, 1.0e-6
        assert_in_delta point.altitude_km, geo.altitude_km, 1.0e-6
      end
    end

    test "honors the :opsmode option" do
      {:ok, tle} = Sidereon.parse_tle(@iss_line1, @iss_line2)
      times = [~U[2024-01-01 12:00:00Z]]

      {:ok, [afspc]} = Sidereon.ground_track(tle, times, opsmode: :afspc)
      {:ok, [improved]} = Sidereon.ground_track(tle, times, opsmode: :improved)

      assert %Sidereon.Geodetic{} = afspc
      assert %Sidereon.Geodetic{} = improved
    end

    test "rejects an invalid opsmode" do
      {:ok, tle} = Sidereon.parse_tle(@iss_line1, @iss_line2)
      times = [~U[2024-01-01 12:00:00Z]]

      assert {:error, {:invalid_option, {:opsmode, :bogus}}} =
               Sidereon.ground_track(tle, times, opsmode: :bogus)
    end

    test "returns an empty track for no datetimes" do
      {:ok, tle} = Sidereon.parse_tle(@iss_line1, @iss_line2)
      assert {:ok, []} = Sidereon.ground_track(tle, [])
    end

    # The ground-track NIF surfaces an SGP4 init failure rather than masking it
    # as {:ok, []}. Init failure is unreachable through parse_tle (the element
    # bridge enforces the same field ranges as SGP4 init), so we drive the NIF
    # boundary directly with a valid element map whose epoch is pushed out of the
    # JD domain SGP4 accepts.
    defp uninitializable_nif_map do
      {:ok, tle} = Sidereon.parse_tle(@iss_line1, @iss_line2)
      {:ok, map} = Sidereon.SGP4.to_nif_elements_map(tle)
      %{map | epochdays: 1.0e9}
    end

    test "the NIF still returns {:ok, []} for empty datetimes even when init would fail" do
      assert {:ok, []} = Sidereon.NIF.ground_track(uninitializable_nif_map(), [], :afspc)
    end

    test "the NIF errors (does not mask as {:ok, []}) when SGP4 init fails for non-empty datetimes" do
      tuples = [{{2024, 1, 1}, {12, 0, 0, 0}}]

      assert {:error, "SGP4 init: " <> _} =
               Sidereon.NIF.ground_track(uninitializable_nif_map(), tuples, :afspc)
    end

    test "returns {:invalid_field, :datetimes, value} for a non-DateTime entry" do
      {:ok, tle} = Sidereon.parse_tle(@iss_line1, @iss_line2)
      times = [~U[2024-01-01 12:00:00Z], :not_a_datetime]

      assert {:error, {:invalid_field, :datetimes, :not_a_datetime}} =
               Sidereon.ground_track(tle, times)
    end
  end
