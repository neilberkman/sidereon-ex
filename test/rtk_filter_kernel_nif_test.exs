defmodule Sidereon.RTKFilterKernelNIFTest do
  ...
  test "updates one RTK filter epoch and carries held ambiguities" do
    ...
    state =
      {{3, [{"G", "G01"}], [], 10_000.0, 0}, {-30.0, 25.0, -10.0}, [], prior_information(10_000.0), [], []}
    ...
    assert {{3, [{"G", "G01"}], ["G01", "G02", "G03", "G04", "G05"], 10_000.0, 1}, baseline, _, _, cycles, metres} =
             next_state
    ...
    assert {{3, [{"G", "G01"}], ["G01", "G02", "G03", "G04", "G05"], 10_000.0, 2}, _baseline, _, _, ^cycles, _metres} =
             second_state
  end
  ...
  test "updates a batch of RTK filter epochs in one NIF call" do
    ...
    assert {{3, [{"G", "G01"}], ["G01", "G02", "G03", "G04", "G05"], 10_000.0, 1}, first_baseline, _, _, cycles,
            _metres} = first_state

    assert distance(first_baseline, truth) < 1.0e-3
    assert cycles == [{"G02", 3}, {"G03", -7}, {"G04", 12}, {"G05", -4}]

    assert {{3, [{"G", "G01"}], ["G01", "G02", "G03", "G04", "G05"], 10_000.0, 2}, second_baseline, _, _, ^cycles,
