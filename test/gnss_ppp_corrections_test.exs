defmodule Sidereon.GNSS.PPPCorrectionsTest do
  ...
  describe "PPPCorrections.build on a real GPS arc" do
    ...
    test "pole tide is an opt-in, bounded displacement table (B1)", ctx do
      # IERS polar motion of the date (ZIM2, 2026/133 ~ arcsec scale).
      pole = %{xp_arcsec: 0.21, yp_arcsec: 0.34}

      {:ok, c} =
        PPPCorrections.build(ctx.sp3, ctx.epochs, ctx.approx, %{pole_tide: pole})

      # The dedicated pole-tide table is populated; it is independent of the
      # solid-earth tide table (which stays empty when not requested).
      assert map_size(c.pole_tide) > 0
      assert c.tide == %{}
      assert c.ocean_loading == %{}

      # Solid-earth pole tide is millimetre-to-centimetre scale.
      for {_epoch, d} <- c.pole_tide do
        assert norm(d) < 0.1
        assert norm(d) > 0.0
      end
    end

    test "ocean loading is an opt-in, bounded displacement table from a BLQ block (B1)", ctx do
      {:ok, c} =
        PPPCorrections.build(ctx.sp3, ctx.epochs, ctx.approx, %{ocean_loading: zim2_blq()})

      assert map_size(c.ocean_loading) > 0
      assert c.tide == %{}
      assert c.pole_tide == %{}

      # Ocean tide loading is centimetre scale at a mid-latitude inland station.
      for {_epoch, d} <- c.ocean_loading do
        assert norm(d) < 0.1
      end
    end

    test "an invalid ocean-loading BLQ block is a tagged error", ctx do
      bad = %{amplitude_m: [[1.0, 2.0]], phase_deg: [[0.0, 0.0]]}

      assert {:error, {:invalid_option, :amplitude_m}} =
               PPPCorrections.build(ctx.sp3, ctx.epochs, ctx.approx, %{ocean_loading: bad})
    end
  ...
  defp pos_shift(a, b), do: :math.sqrt((a.x_m - b.x_m) ** 2 + (a.y_m - b.y_m) ** 2 + (a.z_m - b.z_m) ** 2)
  ...
  # ZIM2 ocean-loading BLQ block (Bos-Scherneck / OSO Chalmers); three rows
  # (radial, west, south), eleven constituents each. Matches the core fixture.
  defp zim2_blq do
    %{
      amplitude_m: [
        [
  ...
  defp naive_to_tuple(%NaiveDateTime{} = ndt),
    do: {{ndt.year, ndt.month, ndt.day}, {ndt.hour, ndt.minute, ndt.second, elem(ndt.microsecond, 0)}}
