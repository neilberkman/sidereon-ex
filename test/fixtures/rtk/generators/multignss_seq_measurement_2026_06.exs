defmodule MultiGNSSSeqMeasurement do
  ...
  def main(args) do
    ...
    base_obs =
      Observations.load!(Path.join(repo, "test/fixtures/obs/WTZR00DEU_R_20201770000_01D_30S_MO_120epoch.rnx"))

    rover_obs =
      Observations.load!(Path.join(repo, "test/fixtures/obs/WTZZ00DEU_R_20201770000_01D_30S_MO_120epoch.rnx"))
    ...
    IO.puts("oracle median #{format_m(reference["mean_truth_error_m"])}, floor #{format_m(floor_m)}")
  ...
  defp format_m(value) when is_number(value), do: :erlang.float_to_binary(value * 1.0, decimals: 6) <> "m"
