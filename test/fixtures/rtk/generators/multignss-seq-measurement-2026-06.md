...
validate-and-document capability: origin/main (sidereon 0.22.0) already ships the
...
- Truth: antenna ARP baseline `rover_arp - base_arp`, marker ECEF adjusted by
  ARP heights (base 0.071 m, rover 0.284 m). The Sidereon ECEF baseline magnitude
...
- Satellite-count / ephemeris mismatch with the oracle is intentional: Sidereon
  ...
  `sidereon_min_sats >= oracle floor`, asserted in the test; per-satellite
  identity and exact per-epoch counts are not asserted.
- The fixed-median truth metric compares the Sidereon ECEF baseline magnitude to
