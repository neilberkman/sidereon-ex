defmodule DopplerVelocityFixture202606 do
  ...
  `test/fixtures/rtk/`:
    ...
    * `doppler_velocity_gsdc_2021_12_15_mtv1_pixel5_l1_gps.nav` - a thinned
      ...
      valid record). The pinned test loads this so `Sidereon.GNSS.Velocity.solve`
  ...
  @arc %{
    ...
    rover_path: "/tmp/gsdc-work/train/2021-12-15-US-MTV-1/GooglePixel5/supplemental/gnss_rinex.21o",
  ...
  defp sanity_check(fixture_dir, samples) do
    ...
    errors =
      ...
      |> Enum.flat_map(fn s ->
        ...
        case Velocity.solve(nav, doppler, epoch, receiver,
               observable: :doppler,
               carrier_hz: @gps_l1_hz
             ) do
          {:ok, sol} ->
    ...
    IO.puts("sanity (thinned NAV): n=#{n} median=#{Float.round(med, 4)} m/s p95=#{Float.round(p95, 4)} m/s")
  end
