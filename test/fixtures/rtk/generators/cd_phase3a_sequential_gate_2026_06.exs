defmodule CDPhase3aSequentialGate202606 do
  @moduledoc false

  # Phase 3a sequential truth gate: the wrong-fix poisoning measured in
  # Phase 2 (151 fixed epochs at 0.768m, RTKLIB oracle 0.107m) is a property
  # of the SEQUENTIAL per-epoch fix-and-hold path. Does applying receiver
  # antenna corrections stop the per-epoch search from locking wrong
  # integers? Two cells on the full vendored PASA/SCOA L1 arc, stock solver,
  # Elixir filter kernel (the Rust kernel refuses `:receiver_antenna_corrections`
  # until the kernel port lands; the kernels are bit-exact gated, so the
  # uncorrected cell doubles as a cross-check against the Phase 2 numbers):
  #   (s1) sequential filter, :elixir kernel, without corrections
  ...
  @elmask_deg 15.0
  @frequency "G01"
  @default_results "/tmp/cd-phase3a-sequential-gate-results.json"
  ...
  def main(args) do
    ...
    filter_opts = filter_opts(initial_baseline)

    cells = [
      {"s1", "sequential elixir, no corrections", filter_opts},
      {"s2", "sequential elixir, with corrections", filter_opts ++ [receiver_antenna_corrections: corrections]}
    ]
  ...
  defp format_m(value) when is_number(value), do: :erlang.float_to_binary(value * 1.0, decimals: 4) <> "m"
...
CDPhase3aSequentialGate202606.main(System.argv())
