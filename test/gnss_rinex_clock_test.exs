defmodule Sidereon.GNSS.RINEX.ClockTest do
  ...
  describe "load!/1 and clock_s/3" do
    test "parse/1 strictly parses satellite records" do
      assert {:ok, clock} = Clock.parse(@clk)
    ...
    test "serializes and writes RINEX clock text", ctx do
      ...
      out =
        Path.join(
          System.tmp_dir!(),
          "sidereon_clock_write_test_#{System.unique_integer([:positive])}.clk"
        )

      on_exit(fn -> File.rm(out) end)

      assert :ok = Clock.write(clock, out)
      assert {:ok, loaded} = Clock.load(out)
      assert loaded.series == clock.series
    end
  end
