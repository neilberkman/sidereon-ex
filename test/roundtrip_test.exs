defmodule Sidereon.RoundtripTest do
  ...
  describe "TLE round-trip" do
    test "ISS TLE round-trips character-exact" do
      ...
      {:ok, el} = Sidereon.Format.TLE.parse(l1, l2)
      {:ok, {gen_l1, gen_l2}} = Sidereon.Format.TLE.encode(el)
  ...
  describe "OMM round-trip" do
    test "all stations OMMs round-trip" do
      ...
      for omm <- omms do
        ...
        encoded = Sidereon.Format.OMM.encode(el)
  ...
  describe "TLE → OMM → TLE cross-format" do
    test "elements survive TLE → OMM → back" do
      ...
      {:ok, el1} = Sidereon.Format.TLE.parse(l1, l2)
      omm = Sidereon.Format.OMM.encode(el1)
      {:ok, el2} = Sidereon.Format.OMM.parse(omm)
