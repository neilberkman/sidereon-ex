defmodule SidereonTest do
  ...
  test "teme_to_gcrs returns position and velocity" do
    ...
    teme_mag =
      :math.sqrt(elem(teme.position, 0) ** 2 + elem(teme.position, 1) ** 2 + elem(teme.position, 2) ** 2)

    gcrs_mag =
      :math.sqrt(elem(gcrs.position, 0) ** 2 + elem(gcrs.position, 1) ** 2 + elem(gcrs.position, 2) ** 2)
