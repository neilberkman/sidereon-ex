defmodule Sidereon.GNSS.PositioningTest do
  ...
  # Canonical-vs-reference clustering bound (meters). Canonical SPP and the
  # reference share the same physics, differing only in operation order (iterative
  # light-time to convergence + a WGS84/ITRF geodetic basis + the owned
  # deterministic trust-region solve), so on a shared case they cluster within the
  # sub-millimetre second-order geodetic-basis perturbation. Mirrors the crate's
  # named CANONICAL_VS_REFERENCE_SPP_TOL_M = 1e-3 m bar (the crate observes ~8e-6 m
  # on its fixture); a divergence beyond this band is a bug to root-cause, not a
  # band to widen.
  @canonical_vs_reference_bound_m 1.0e-3
  # Canonical truth bound (meters): the reference recovers the synthesized truth to
  # @agreement_bound_m and canonical clusters within @canonical_vs_reference_bound_m
  # of the reference, so canonical recovers truth within their sum.
  @canonical_truth_bound_m @agreement_bound_m + @canonical_vs_reference_bound_m
  ...
  describe "solve/4 canonical strategy selection (opt-in)" do
    test "the default selection is the reference strategy, bit-for-bit", ctx do
      base = canonical_base_opts(ctx)

      assert {:ok, %Solution{} = default} =
               Positioning.solve(ctx.sp3, ctx.observations, @epoch, base)

      assert {:ok, %Solution{} = reference} =
               Positioning.solve(
                 ctx.sp3,
                 ctx.observations,
                 @epoch,
      ...
      # Omitting :strategy and asking for :reference must be the identical path,
      # to the bit. This is the additive invariant: canonical selection does not
      # disturb the default reference-faithful result.
      assert default.position.x_m === reference.position.x_m
      assert default.position.y_m === reference.position.y_m
      assert default.position.z_m === reference.position.z_m
      assert default.rx_clock_s === reference.rx_clock_s
    end

    test "canonical is selectable, deterministic, and clusters within the named band of the reference",
         ctx do
      base = canonical_base_opts(ctx)
      ...
      assert {:ok, %Solution{} = canonical} =
               Positioning.solve(
                 ctx.sp3,
                 ctx.observations,
                 @epoch,
                 base ++ [strategy: :canonical]
               )

      # Selecting canonical returns DIFFERENT numbers than the reference (the
      # rigorous op-order genuinely diverges), so this is not a silent forward.
      assert {canonical.position.x_m, canonical.position.y_m, canonical.position.z_m} !=
               {reference.position.x_m, reference.position.y_m, reference.position.z_m}

      # Determinism: a second canonical solve reproduces the first bit-for-bit.
      assert {:ok, %Solution{} = canonical_again} =
               Positioning.solve(
                 ctx.sp3,
                 ctx.observations,
                 @epoch,
      ...
      assert canonical.position.x_m === canonical_again.position.x_m
      assert canonical.position.y_m === canonical_again.position.y_m
      assert canonical.position.z_m === canonical_again.position.z_m
      assert canonical.rx_clock_s === canonical_again.rx_clock_s

      # Bounded tolerance: canonical clusters within the named band of the
      # reference (same physics, op-order only).
      assert_in_delta canonical.position.x_m,
                      reference.position.x_m,
                      @canonical_vs_reference_bound_m
      ...
      # Truth: canonical recovers the synthesized receiver truth.
      [tx, ty, tz, _tb] = ctx.truth_x
      assert_in_delta canonical.position.x_m, tx, @canonical_truth_bound_m
      assert_in_delta canonical.position.y_m, ty, @canonical_truth_bound_m
      assert_in_delta canonical.position.z_m, tz, @canonical_truth_bound_m

      assert canonical.metadata.converged
    end
    ...
    test "an unknown strategy is rejected with a tagged error", ctx do
      assert Positioning.solve(
               ...
               ctx.observations,
               @epoch,
               canonical_base_opts(ctx) ++ [strategy: :bogus]
             ) == {:error, {:invalid_option, :strategy}}
    end

    test "canonical is refused together with the robust FDE path", ctx do
      assert Positioning.solve(
               ctx.sp3,
               ctx.observations,
               @epoch,
               canonical_base_opts(ctx) ++ [strategy: :canonical, robust: true]
  ...
  describe "solve/4 from broadcast ephemeris" do
    ...
    test "loads a GLONASS navigation file through the broadcast path" do
      ...
      assert %Sidereon.GNSS.Broadcast{} = Sidereon.GNSS.Broadcast.load!(@nav_glonass_path)
    end
    ...
    test "recovers a known receiver from broadcast GLONASS pseudoranges" do
      eph = Sidereon.GNSS.Broadcast.load!(@nav_glonass_path)
    ...
    test "a GLONASS ionosphere-corrected solve is accepted when the FDMA channel map is supplied" do
      ...
      channels =
        eph
        |> Sidereon.GNSS.Broadcast.glonass_records()
        |> Map.new(fn rec ->
    ...
    test "loads a RINEX 4.00 navigation file through the broadcast path" do
      ...
      assert {:ok, %Sidereon.GNSS.Broadcast{}} =
               Sidereon.GNSS.Broadcast.parse(File.read!(@nav_v4_path))

      assert %Sidereon.GNSS.Broadcast{} = Sidereon.GNSS.Broadcast.load!(@nav_v4_path)
    end

    test "recovers a known receiver from broadcast GPS pseudoranges" do
      eph = Sidereon.GNSS.Broadcast.load!(@nav_path)
  ...
  # The base solve options shared by the reference and canonical solves in the
  # strategy-selection describe; only the :strategy selector differs between them.
  defp canonical_base_opts(ctx) do
    [
      ionosphere: true,
      troposphere: true,
      klobuchar_alpha: ctx.alpha,
