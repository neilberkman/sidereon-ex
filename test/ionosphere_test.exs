defmodule Sidereon.GNSS.IonosphereTest do
  ...
  describe "galileo_nequick_g_delay/7" do
    ...
    test "an E5a carrier delay exceeds the E1 delay (dispersive 1/f^2 scaling)" do
      ...
      {:ok, e1} =
        Ionosphere.galileo_nequick_g_delay(@nequick_coeffs, 47.0, 8.0, 122.0, 37.0, epoch, 1_575_420_000.0)

      {:ok, e5a} =
        Ionosphere.galileo_nequick_g_delay(@nequick_coeffs, 47.0, 8.0, 122.0, 37.0, epoch, 1_176_450_000.0)
    ...
    test "azimuth does not change the NeQuick-G delay (slant by elevation only)" do
      ...
      {:ok, a} =
        Ionosphere.galileo_nequick_g_delay(@nequick_coeffs, 47.0, 8.0, 0.0, 37.0, epoch, 1_575_420_000.0)

      {:ok, b} =
        Ionosphere.galileo_nequick_g_delay(@nequick_coeffs, 47.0, 8.0, 270.0, 37.0, epoch, 1_575_420_000.0)
