defmodule Sidereon.OMMTest do
  ...
  describe "parse/1" do
    test "parses ISS OMM record", %{iss_omm: omm} do
      {:ok, tle} = Sidereon.Format.OMM.parse(omm)
    ...
    test "parses all station OMMs", %{omms: omms} do
      results = Enum.map(omms, &Sidereon.Format.OMM.parse/1)
  ...
  describe "encode/1" do
    test "round-trips through OMM", %{iss_omm: original} do
      {:ok, tle} = Sidereon.Format.OMM.parse(original)
      omm = Sidereon.Format.OMM.encode(tle)
  ...
  describe "text OMM parsing" do
    ...
    test "parse/1 auto-detects text encodings" do
      assert {:ok, %OMM{norad_cat_id: 25544}} = OMM.parse(core_omm_fixture("25544", "kvn"))
      assert {:ok, %OMM{norad_cat_id: 25544}} = OMM.parse(core_omm_fixture("25544", "xml"))
      assert {:ok, %OMM{norad_cat_id: 25544}} = OMM.parse(core_omm_fixture("25544", "json"))
    end
  end
  ...
  describe "text OMM serialization" do
    ...
    test "core-style to_*_string aliases serialize typed OMMs" do
      ...
      assert Jason.decode!(json)["NORAD_CAT_ID"] == 28884
    end
  end
  ...
  describe "typed OMM conversion" do
    test "to_elements/1 matches the matching TLE fixture" do
      ...
      {:ok, tle_elements} = Sidereon.Format.TLE.parse(line1, line2)
