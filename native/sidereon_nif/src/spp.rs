...
//! [`SolveInputs`], drives the shared [`estimate`] selector under the requested
//! strategy (reference by default, canonical opt-in), and encodes the
...
use sidereon_core::{
    ...
    positioning::{
        solve_with_fallback, BroadcastReason, Corrections, EphemerisSource, FallbackError,
        FixSource, KlobucharCoeffs, Observation, ReceiverSolution, RejectionReason, RobustConfig,
        SolveInputs, SolvePolicy, SolvePolicyError, SourcedSolution, SppError, SurfaceMet,
...
use crate::strategy::{decode_strategy, StrategySelection};
...
/// Run the solve against any ephemeris source under the selected strategy and
/// encode the result term. Drives the shared [`estimate`] selector: `Reference`
/// resolves to the SPP reference strategy (byte-identical to the legacy
/// `solve_with_policy` path, which is itself an `estimate(.., reference)` call),
/// `Canonical` resolves to the canonical SPP strategy (iterative light-time /
/// WGS84 / owned deterministic solver).
fn solve_to_term<'a>(
    env: Env<'a>,
    ...
    let options = EstimateOptions::new(strategy.strategy_id(Technique::Spp));
    match estimate(
        EstimateInput::Spp {
            eph,
            inputs,
