...
use sidereon_core::atmosphere::ionosphere::{
    ionex_slant_delay, ionosphere_delay, klobuchar_native, GalileoNequickCoeffs, Ionex, IonoModel,
    KlobucharParams,
};
...
/// latitude/longitude and the satellite azimuth/elevation arrive in radians (the
/// Elixir wrapper converts its degree inputs); the epoch arrives as the split
...
/// the same instant with no second representation. `azimuth_rad` is validated by
...
fn galileo_nequick_g_delay(
    ...
    let receiver =
        Wgs84Geodetic::new(lat_rad, lon_rad, 0.0).map_err(crate::errors::invalid_input)?;
    let split =
