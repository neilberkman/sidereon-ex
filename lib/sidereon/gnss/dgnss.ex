defmodule Sidereon.GNSS.DGNSS do
  ...
  runs `Sidereon.GNSS.Positioning.solve/4` on the corrected pseudoranges with
  `ionosphere: false` and `troposphere: false` forced on (the differential has
  already removed those delays, so caller-supplied values for those two options
  ...
  defp observation_terms(observations),
    do: Enum.map(observations, fn {sat, pr} -> {sat, pr / 1.0} end)
end
