defmodule Sidereon.GNSS.PPPCorrections do
  ...
  @spec empty() :: t()
  def empty, do: %{tide: %{}, windup_m: %{}, sat_pco_ecef: %{}, sat_pcv_m: %{}, pole_tide: %{}, ocean_loading: %{}}
  ...
  defp pole_tide_term(nil), do: {:ok, nil}

  defp pole_tide_term(%{xp_arcsec: xp, yp_arcsec: yp}) when is_number(xp) and is_number(yp),
    do: {:ok, {xp / 1.0, yp / 1.0}}

  defp pole_tide_term(_other), do: {:error, {:invalid_option, :pole_tide}}

  defp ocean_loading_term(nil), do: {:ok, nil}

  defp ocean_loading_term(%{amplitude_m: amplitude, phase_deg: phase}) when is_list(amplitude) and is_list(phase) do
    with {:ok, amplitude} <- blq_rows(amplitude, :amplitude_m),
         {:ok, phase} <- blq_rows(phase, :phase_deg) do
      {:ok, {amplitude, phase}}
    end
  end

  defp ocean_loading_term(_other), do: {:error, {:invalid_option, :ocean_loading}}

  # Three rows (radial, west, south); the core validates the per-row coefficient
  # count, so this only coerces to floats and forwards.
  defp blq_rows(rows, field) when length(rows) == 3 do
    if Enum.all?(rows, &(is_list(&1) and Enum.all?(&1, fn v -> is_number(v) end))) do
  ...
  defp blq_rows(_rows, field), do: {:error, {:invalid_option, field}}
  ...
  defp epoch_terms(_epochs, _needs_observation_frequency?), do: {:error, {:invalid_argument, :epochs}}
  ...
  defp observation_term(observation, _needs_frequency?), do: {:error, {:invalid_observation, observation}}
  ...
  defp valid_satellite_id?(<<system::binary-size(1), prn::binary>>) when system in ~w(G R E C J I S) do
