defmodule Sidereon.GNSS.CarrierPhase do
  ...
  def melbourne_wubbena(phi1_cyc, phi2_cyc, p1_m, p2_m, f1, f2)
      when is_number(phi1_cyc) and is_number(phi2_cyc) and is_number(p1_m) and is_number(p2_m) and is_number(f1) and
             is_number(f2) do
  ...
  def code_minus_carrier(p_m, phi_cyc, f_hz) when is_number(p_m) and is_number(phi_cyc) and is_number(f_hz) do
  ...
  defp validate_slip_opts!(opts) do
    ...
    {non_negative!(:gf_threshold_m, gf), non_negative!(:mw_threshold_cycles, mw), non_negative!(:min_arc_gap_s, gap)}
  end
