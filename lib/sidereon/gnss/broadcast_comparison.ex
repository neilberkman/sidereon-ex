defmodule Sidereon.GNSS.BroadcastComparison do
  ...
  The same call works against a broadcast product by calling
  `compare_broadcast_vs_precise/4` directly with loaded products.
  ...
  def compare(%Broadcast{handle: broadcast}, %SP3{handle: precise}, sat_ids, %{from: from, to: to, step_s: step_s})
      when is_list(sat_ids) do
    # The interface picks the two start anchors; the core driver builds the
    # regular per-epoch grid and the velocity neighbours. The broadcast axis is a
    # continuous J2000 second (the value `Sidereon.GNSS.Broadcast.position`
    # marshals); the precise axis is a split Julian date advanced in lockstep.
    ...
    {overall, per_satellite, missing} =
      NIF.broadcast_comparison(
        ...
        precise,
        sat_ids,
        broadcast_t0_j2000_s,
        broadcast_t1_j2000_s,
