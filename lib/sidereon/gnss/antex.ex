defmodule Sidereon.GNSS.Antex do
  ...
  def parse(text) when is_binary(text) do
    case NIF.antex_parse(text) do
      {:ok, rows} -> {:ok, %__MODULE__{antennas: decode_antennas(rows)}}
  ...
  defp antenna_term(%Antenna{} = antenna) do
    {{antenna.id, encode_kind(antenna.kind), antenna.type, antenna.serial},
     {antenna.dazi_deg, antenna.zenith_start_deg, antenna.zenith_end_deg,
      antenna.zenith_step_deg},
     {antenna.sinex_code, encode_datetime(antenna.valid_from),
      encode_datetime(antenna.valid_until)}, frequency_terms(antenna.frequencies)}
  end
