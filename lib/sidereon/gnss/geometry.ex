  ...
  *after* inverting, by rotating the 3x3 position block into the local ENU frame
  ...
  defp dop_options(opts) do
    with {:ok, {elevation_mask_deg, systems, extrapolate?}} <- visibility_options(opts),
      ...
      {:ok, {elevation_mask_deg, systems, weighting, light_time?, use_explicit?, satellites, extrapolate?}}
    end
  end
  ...
  defp validate_step_seconds(step_seconds) when is_integer(step_seconds) and step_seconds > 0, do: {:ok, step_seconds}
