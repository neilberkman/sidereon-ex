defmodule Sidereon.GNSS.RINEX.Clock do
  ...
  @doc """
  Parse RINEX clock text strictly.

  Returns `{:ok, %Sidereon.GNSS.RINEX.Clock{}}` or `{:error, reason}`. Malformed
  satellite clock rows are rejected.
  """
  ...
  def clock_s(%__MODULE__{series: series}, satellite_id, %NaiveDateTime{} = epoch) when is_binary(satellite_id) do
  ...
  @doc """
  Serialize clock records to RINEX clock text.
  """
  @spec to_rinex_string(t()) :: {:ok, String.t()} | {:error, term()}
  def to_rinex_string(%__MODULE__{series: series}) do
    case NIF.rinex_clock_to_string(Map.to_list(series)) do
      {:ok, text} -> {:ok, text}
      {:error, reason} -> {:error, reason}
    end
  rescue
