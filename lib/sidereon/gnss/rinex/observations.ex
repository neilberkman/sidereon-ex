defmodule Sidereon.GNSS.RINEX.Observations do
  ...
  upstream by `Sidereon.GNSS.Data.fetch/2`, so this module only ever sees plain text
  or CRINEX text.
      ...
      {:ok, rows} ->
        {:ok,
         Map.new(rows, fn {sat, phases} ->
           {sat,
            Enum.map(phases, fn
              {code, value_cycles, lli, ssi,
               {frequency_hz, wavelength_m, value_m, phase_shift_cycles}} ->
                %{
                  code: code,
