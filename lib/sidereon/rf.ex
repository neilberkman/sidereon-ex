defmodule Sidereon.RF do
  ...
  The link-budget formulas live in the `sidereon-core` Rust core; this module
  ...
  Each distance is evaluated by the same core helper as `fspl/2`.
  """
  @spec fspl_batch([number()], number()) :: [float()]
  def fspl_batch(distances_km, frequency_mhz) when is_list(distances_km) do
    NIF.rf_fspl_batch(Enum.map(distances_km, &(&1 / 1.0)), frequency_mhz / 1.0)
  end
  ...
  @doc """
  Batch link margin in dB.

  `budgets` is a list of maps accepted by `link_margin/1`.
  """
  @spec link_margin_batch([map()]) :: [float()]
  def link_margin_batch(budgets) when is_list(budgets) do
    budgets
    |> Enum.map(fn %{
                     eirp_dbw: eirp_dbw,
                     fspl_db: fspl_db,
      ...
      {eirp_dbw / 1.0, fspl_db / 1.0, gt / 1.0, losses / 1.0, required / 1.0}
    end)
    |> NIF.rf_link_margin_batch()
  end
