defmodule Sidereon.Passes do
  ...
  @allowed_options [:min_elevation, :step_seconds]
  ...
  defp validate_options(opts) when is_list(opts) do
    if Keyword.keyword?(opts) do
      with :ok <- validate_option_keys(opts),
           ...
           {:ok, step_seconds} <- optional_positive_integer(opts, :step_seconds, 60) do
        {:ok, %{min_elevation: min_elevation, step_seconds: step_seconds}}
      end
    else
